...
public class LastValuesFunctionIT extends ParallelStatsDisabledIT {
    ...
    public void simpleTest() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(val, 3) WITHIN GROUP (ORDER BY dates DESC) FROM " + tableName
                        + " GROUP BY page_id");

        assertTrue(rs.next());
        Assert.assertArrayEquals(new int[] {7, 9, 4}, (int[]) rs.getArray(1).getArray());
        assertTrue(rs.next());
        Assert.assertArrayEquals(new int[] {10, 13, 8}, (int[]) rs.getArray(1).getArray());
        assertFalse(rs.next());
    }
    ...
    public void varcharDatatypeSimpleTest() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(\"value\", 2) WITHIN GROUP (ORDER BY \"DATE\" ASC) FROM " + table_name
                        + " GROUP BY page_id");

        assertTrue(rs.next());
        Assert.assertArrayEquals(new String[] {"4", "2"}, (String[]) rs.getArray(1).getArray());
        assertFalse(rs.next());
    }
    ...
    public void floatDatatypeSimpleTest() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(\"value\", 2) WITHIN GROUP (ORDER BY \"DATE\" ASC) FROM " + tableName
                        + " GROUP BY page_id");

        assertTrue(rs.next());
        assertEquals(((float[]) rs.getArray(1).getArray())[0], 400, 0.000001);
        assertEquals(((float[]) rs.getArray(1).getArray())[1], 2, 0.000001);
        assertFalse(rs.next());
    }
    ...
    public void doubleDatatypeSimpleTest() throws Exception {
        ...
        assertTrue(rs.next());
        assertEquals(((double[]) rs.getArray(1).getArray())[0], 400, 0.000001);
        assertEquals(((double[]) rs.getArray(1).getArray())[1], 2, 0.000001);
        assertFalse(rs.next());
    }
    ...
    public void offsetValueAscOrder() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(\"value\", 2)  WITHIN GROUP (ORDER BY \"DATE\" ASC) FROM "
    ...
    public void simpleTestNoGroupBy() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(val, 2) WITHIN GROUP (ORDER BY dates DESC) FROM " + tableName);
    ...
    public void offsetValueDescOrder() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(\"value\", 2)  WITHIN GROUP (ORDER BY \"DATE\" DESC) FROM "
                        + tableName + " GROUP BY page_id");
    ...
    public void offsetValueSubAggregation() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(SUM_VALUE, 2) WITHIN GROUP (ORDER BY MIN_DATE ASC) FROM (" +
                        "SELECT MIN(\"DATE\") AS MIN_DATE, SUM(\"value\") AS SUM_VALUE FROM "
                        + tableName + " GROUP BY page_id) x");

        assertTrue(rs.next());
        Assert.assertArrayEquals(new long[] {152, 13}, (long[]) rs.getArray(1).getArray());
        assertFalse(rs.next());
    }
    ...
    public void offsetValueLastMismatchByColumn() throws Exception {
        ...
        assertTrue(rs.next());
        Assert.assertArrayEquals(new long[] {1, 9}, (long[]) rs.getArray(1).getArray());
        assertFalse(rs.next());
    }
    ...
    public void testSortOrderInDataColWithOffset() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(\"value\", 2)  WITHIN GROUP (ORDER BY dates ASC) FROM "
                        + tableName + " GROUP BY page_id");

        assertTrue(rs.next());
        Assert.assertArrayEquals(new long[] {158, 5}, (long[]) rs.getArray(1).getArray());
        assertFalse(rs.next());
    }
    ...
    public void nonUniqueValuesInOrderByAsc() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(val, 3) WITHIN GROUP (ORDER BY dates ASC) FROM " + tableName
                        + " GROUP BY page_id");
    ...
    public void nonUniqueValuesInOrderByAscSkipDuplicit() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(val, 5) WITHIN GROUP (ORDER BY dates ASC) FROM " + tableName
                        + " GROUP BY page_id");
    ...
    public void ignoreNullValues() throws Exception {
        ...
        conn.createStatement().execute("UPSERT INTO " + tableName
                + " (id, page_id, dates, \"value\") VALUES (5, 8, 4, NULL)");
        conn.createStatement().execute("UPSERT INTO " + tableName
        ...
        assertTrue(rs.next());
        Assert.assertArrayEquals(new long[] {5, 1}, (long[]) rs.getArray(1).getArray());
        assertFalse(rs.next());
    }
    ...
    public void rowLessThanOffsetWithGroupBy() throws Exception {
        ...
        ResultSet rs = conn.createStatement().executeQuery(
                "SELECT LAST_VALUES(val, 2)  WITHIN GROUP (ORDER BY dates ASC) FROM "
                        + tableName + " GROUP BY page_id");

        assertTrue(rs.next());
        int[] result = (int[]) rs.getArray(1).getArray();
        assertEquals(result[0], 3);
        assertInIntArray(new int[] {2, 4}, result[1]);
        assertTrue(rs.next());
